...
BoundaryIntegratorOpenTotalPressure::BoundaryIntegratorOpenTotalPressure
(
...
void BoundaryIntegratorOpenTotalPressure::computeRightState(const Vector& state1, Vector& state2, const Vector& nor) 
{
   ...
   double rhoe1 = state2[num_equation-1];
   double epsIn = state2[1] > 0 ? (rhoe1 - 0.5*rho2U2) / state2[0] : gas_constant / (specific_heat_ratio - 1.0) * TFix;
...
// 
   ...
   // double MTTotSq = rhoU2 / state2[0] / specific_heat_ratio / gas_constant / TFix;

   // double MSq = MTTotSq / (1.0 - 0.5 * (specific_heat_ratio - 1.0) * MTTotSq);

   // //totalPressure
   // double p = pTotal * pow( 1.0 + 0.5 * (specific_heat_ratio - 1.0)*MSq, - specific_heat_ratio / (specific_heat_ratio - 1.0));

   // double T = TFix / ( 1.0 + 0.5 * (specific_heat_ratio - 1.0)*MSq );
